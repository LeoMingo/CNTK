        ...
        if (!fr.IsOneColumnWrt(InputRef(0).GetMBLayout()))
        {
            ...
            if (inputIndex == 0)
            {
                ...
                if (fr.IsBatchMatmul(InputRef(0).GetMBLayout()) &&  fr.IsBatchMatmul(InputRef(1).GetMBLayout()) && rankA <= 2 && rankB <= 2)
                {
                    ...
                    if (!m_transpose)
                        Matrix<ElemType>::BatchMatMul(outputGradient, false, n, input1, true, m, input0Gradient, true);
                    else
                        Matrix<ElemType>::BatchMatMul(input1, false, m, outputGradient, true, n, input0Gradient, true);
                    return;
                }
            }
